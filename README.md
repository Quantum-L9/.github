# Quantum-L9 — org defaults & governance v5.0

Advisory-first governance surface for the Quantum-L9 constellation, plus the
GitHub-native organization defaults every repository inherits. Everything here
**reports and remediates**; nothing blocks until explicitly promoted. See
`docs/ADVISORY.md` for the promotion ladder and `docs/BOUNDARIES.md` for what this
repo deliberately does not do.

## Scope boundary (read this first)

This repo owns **governance metadata, advisory reporting, policy enforcement,
and the GitHub-native organization defaults**. It does not run tests, lint,
typecheck, scan code, diagnose CI failures, or distribute CI implementation.

| Concern | Owner |
| --- | --- |
| Test / lint / build execution | `l9-ci-sdk`, `l9-ci-core` |
| CI targeting, versioning, reconciliation, enforcement | `l9-ci-control-plane` |
...
| --- | --- | --- | --- |
| Org rulesets (advisory) | `rulesets/*.json` | `evaluate` | No |
| Secret scanning | `scripts/enable-secret-scanning.sh` → org API | alerts only | No |
...
| Boundary validation | `ops/validate-starters.sh` | fail-closed | Yes |
...
templates/
├── CODEOWNERS.repo                      manual copy source for consumer repos
├── dependabot.yml                       manual copy source for consumer repos
├── governance-caller.yml                calls org governance workflows
...
├── validate-starters.sh                 boundary validation (rejects retired CI surfaces)
...
```bash
# Validate the boundary and SHA pins before pushing
make validate

# Full org health check (read-only)
make preflight
```

## Consumer repos

Consumer repos receive the GitHub-native defaults automatically by
inheritance. No action required. They can:

- **Opt out of policy enforcement:** Create `.l9/no-policy-enforcement` in the repo
- **Copy non-inheritable files:** Take what a repo needs from `templates/` by hand
  (`CODEOWNERS`, `dependabot.yml`, labels, issue/PR templates)
...
Promotion is a deliberate, documented decision recorded in `docs/adr/`.
